...
# # Add project root to path if not already therepip show flask
...
# Import display emulator for image API
try:
    from kitchenradio.radio.hardware.display_emulator import DisplayEmulator
    DISPLAY_EMULATOR_AVAILABLE = True
except ImportError:
    DISPLAY_EMULATOR_AVAILABLE = False
    print("Warning: Display emulator not available")
...
class KitchenRadioWebServer:
    ...
    def __init__(self, host='0.0.0.0', port=5000, debug=False):
        ...
        # Initialize display emulator for image API
        self.display_emulator = None
        if DISPLAY_EMULATOR_AVAILABLE:
            try:
                self.display_emulator = DisplayEmulator()
                self.display_emulator.initialize()
                logger.info("Display emulator initialized for image API")
            except Exception as e:
                logger.warning(f"Failed to initialize display emulator: {e}")
                self.display_emulator = None
    ...
    def _stop_daemon(self):
        ...
        # Also cleanup display emulator
        if self.display_emulator:
            try:
                self.display_emulator.cleanup()
                logger.info("✅ Display emulator cleaned up")
            except Exception as e:
                logger.warning(f"⚠️ Error cleaning up display emulator: {e}")
    ...
    def _setup_routes(self):
        ...
        @self.app.route('/api/display/image')
        def api_display_image():
            """Get current display image data"""
            if not self.display_emulator:
                return jsonify({
                    'success': False,
                    'error': 'Display emulator not available'
                }), 503
            
            try:
                # Get image data from display emulator
                image_data = self.display_emulator.get_image_data()
                statistics = self.display_emulator.get_statistics()
                ...
                return jsonify({
                    ...
                    'display_info': self.display_emulator.get_display_info()
                })
        ...
        @self.app.route('/api/display/ascii')
        def api_display_ascii():
        ...
        def api_display_test():
            """Test the display with a pattern"""
            if not self.display_emulator:
                return jsonify({
                    'success': False,
                    'error': 'Display emulator not available'
            ...
            try:
                success = self.display_emulator.test_display()
                return jsonify({
                    'success': success,
                    'message': 'Test pattern displayed' if success else 'Test failed'
                })
                
            except Exception as e:
                logger.error(f"Error testing display: {e}")
                return jsonify({
                    'success': False,
                    'error': str(e)
                }), 500
